 ...
 * Description: La Caja de Herramientas de Víctor Campuzano. Un plugin construido por una comunidad con herramientas y funciones que te ayudarán a hacer más satisfactoria tu experiencia como Blogger. Por favor, visita <a href="https://vicampuzano.com/" target="_blank">vicampuzano.com</a> para más información o contactar conmigo.
 * Version: 1.9.32
 * Author: Víctor Campuzano (vcgs)
 * Author URI: https://vicampuzano.com/
 * Config: ¿Qué se debería poner aquí?
 * Config URI: https://vicampuzano.com/
 * License: GPL2
 */
...
function registra_fontawesome() {
	...
	wp_register_script( 'fontawesome', 'https://use.fontawesome.com/1fda31d217.js', array( 'jquery' ), false, false );
...
if (is_page() && $options['bs_activate']==1)
{
	function registra_bootstrap() {
		wp_register_style('bootstrap_css','https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta.2/css/bootstrap.min.css', false, false, 'all');
		wp_register_script( 'bootstrap_js', 'https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta.2/js/bootstrap.min.js', array( 'jquery' ), false, false );
...
if ($options['pp_activate']==1)
{
	...
	function carga_piopialo() {
		wp_register_script( 'piopialo', plugins_url( '/js/piopialo.js' , __FILE__ ), array( 'jquery' ), '1.9.32', true );
	...
	if ($content != null) {
		...
		// Codificar el texto.
		$texto = rawurlencode('"'.substr(strip_tags($content),0,256-strlen($ivia)).'" '.$ivia.' ');
		...
		//localizamos el usuario de Twitter
		preg_match("/(?<=^|\\s)@([a-z0-9_]+)/i", $ivia, $usernames);
		$solotuit = 'http://www.twitter.com/intent/tweet/?text='.$texto.'&url='.$miurl.'&related='.rawurlencode('vicampuzano,'.$usernames[0]);
		...
		if ($i_facebook && !$esfeed)
		{
			...
			$enlace .= '&nbsp;&nbsp;<a target="_blank" class="pioface" data-piolink="http://www.facebook.com/sharer/sharer.php?u='.$miurl.'&quote='.rawurlencode(strip_tags($content)).'" title="En Facebook"><i class="fa fa-facebook-square">&nbsp;</i></a>';
		}
			...
			if ($itheme == 'original' || $itheme == '') 
			{
				...
				$returnval = $ancla.'<div class="piopialob">
									...
									<div class="piopialob-text">
												<span class="piopialob-frase"  data-piolink="'.$solotuit.'">'.strip_tags($content).'</span><span class="piopialob-link">'.$enlace.'</span>
									</div>';
				if ($powered) {
				...
				$returnval = $ancla.'<div class="piopialobred">
									<div class="piopialobred-text">
												<span class="piopialobred-frase" data-piolink="'.$solotuit.'">'.strip_tags($content).'</span><span class="piopialobred-link">'.$enlace.'</span>
									</div>';
				if ($powered) {
				...
				$returnval = $ancla.'<div class="piopialobmod">
									<div class="piopialobmod-text">
												<span class="piopialobmod-frase" data-piolink="'.$solotuit.'">'.strip_tags($content).'</span><span class="piopialobmod-link">'.$enlace.'</span>
									</div></div>';
				if ($powered) {
	...
	//			$returnval .= '</div>';
				...
				$returnval = $ancla.'<div class="piopialobmodgris hvr-bounce-to-bottom">
									<div class="piopialobmodgris-text">
												<span class="piopialobmodgris-frase" data-piolink="'.$solotuit.'">'.strip_tags($content).'</span><span class="piopialobmodgris-link">'.$enlace.'</span>
									</div></div>';
				if ($powered) {
...
function modificar_comentario( $text ){
	...
	$ivia = $options['pp_via']; // Texto de firma / mención del Tuit
	//localizamos el usuario de Twitter
		preg_match("/(?<=^|\\s)@([a-z0-9_]+)/i", $ivia, $authusername);
	// Obtener el Usuario de Twitter del que comenta
	if( $commenttwitter = get_comment_meta( get_comment_ID(), 'twitter', true ) ) {
		// Vamos a añadir related 
		
		//localizamos el usuario de Twitter
		preg_match("/(?<=^|\\s)@([a-z0-9_]+)/i", esc_attr($commenttwitter), $commentusername);
		$tuitear=rawurlencode('Me ha gustado el comentario de @'.esc_attr($commenttwitter).' en este post '.$ivia);
	}
	else { 
		$tuitear=rawurlencode('Me ha gustado este comentario en un post '.$ivia);
	}
	
	// Obtener la URL directa del comentario
	$url = urlencode(get_comment_link(get_comment_ID()));
	if (is_admin() || is_feed()) {
		$enlace = '<p><a class="piopialo-comment" href="http://www.twitter.com/intent/tweet/?text='.$tuitear.'&url='.$url.'&related='.rawurlencode($authusername[0].','.$commentusername[0]).'"  title="Piopia este comentario"><i class="fa fa-twitter"></i> Citar Comentario</a></p>';
	} else {
		$enlace = '<p class="comment-quote-link comment-piolink"><span class="piopialo-comment" data-piolink="http://www.twitter.com/intent/tweet/?text='.$tuitear.'&url='.$url.'&related='.rawurlencode($authusername[0].','.$commentusername[0]).'"  title="Pincha aquí para compartir un enlace directo a este comentario"><i class="fa fa-twitter"></i> Citar Comentario</span></p>';
	}
...
// Función de citar los comentarios en Facebook
if($options['pp_fco']=="1")
{
add_filter( 'comment_text', 'modificar_comentario_facebook');
function modificar_comentario_facebook( $text ){
    ...
    $cauthor = get_comment_author($cid);
    
	// Obtener la URL directa del comentario
	$url = urlencode(get_comment_link($cid));
    // Obtener el texto "en bruto" del comentario
    $ctext = rawurlencode($cauthor.' ha comentado: '.strip_tags(get_comment_text($cid)));
	if (is_admin() || is_feed()) {
		$enlace = '<p><a class="facecita-comment" href="https://www.facebook.com/sharer/sharer.php?u='.$url.'&quote='.$ctext.'" title="Cita este compentario en Facebook con un link directo"><i class="fa fa-facebook"></i> Citar Comentario</a></p>';    
	} else {
		$enlace = '<p class="comment-quote-link comment-facelink"><span class="piopialo-comment" data-piolink="https://www.facebook.com/sharer/sharer.php?u='.$url.'&quote='.$ctext.'" title="Cita este comentario en Facebook con un link directo"><i class="fa fa-facebook"></i> Citar Comentario</span></p>';
	}
...
if($options['textareafirst'] == '1') {
	global $vcgscf_textarea;
	$vcgscf_textarea = 'asereje';
	
	// Recogemos el valor del textarea
	add_filter('comment_form_defaults', 'vcgscf_savetextarea');
	
	// Filtramos el area de comentario
	add_filter('comment_form_field_comment','vcgscf_quitadefault');
	
	// Añadimos el textarea en el principio del formulario
	add_action('comment_form_top','vcgscf_addtextarea');
	
	function vcgscf_savetextarea( $fields = array() ) {
		global $vcgscf_textarea;
		$vcgscf_textarea = $fields['comment_field'];
		$fields['comment_field'] = '';
		return $fields;
	}
	
	function vcgscf_quitadefault($pordefecto = '') {
		return '';
	}
	
	function vcgscf_addtextarea($input=array()) {
		global $vcgscf_textarea;
		echo $vcgscf_textarea;
		return $input;
	}
}
