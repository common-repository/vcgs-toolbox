	 ...
	 public function missing_reply_display( $column_name = '', $comment_id = 0 ) {
		...
		if( $this->comment_is_by_post_author( $comment_id ) ) {
		 ...
		 // Otherwise, let's look at the replies to determine if the author has made a reply
			...
			if( ($this->author_has_replied( $replies )) || !(get_comment_meta( $comment_id, '_cnrt_missing', true )) ) 
			{
				...
				$currenturl = "edit-comments.php?comment_status=all&missing_reply=1&comment_type=comment&markreplied=".$comment_id;
				$message = '<a href="'.$currenturl.'" title="Pincha en este enlace si quieres que este comentario no aparezca en la lista de pendientes de responder">NO</a>';
